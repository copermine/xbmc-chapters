...
class GUI( xbmcgui.WindowXMLDialog ):
    ...
    def init_chapters( self ):
        ...
        for chapter in chapters.get_chapters():
            li = xbmcgui.ListItem(label=chapter[ 'ChapterName' ], thumbnailImage=chapter[ 'ChapterThumb' ])
            li.setLabel2( chapter[ 'ChapterName2' ] )
            ...
            if chapter['ChapterThumb']  <> '' and not hasThumb: 
                hasThumb = True
                self.getControl( 30001 ).setLabel( "hasthumb" ) # set fake label before switching views
            if chapter['ChapterName2']  <> '' and not hasLabel2: 
                hasLabel2 = True
